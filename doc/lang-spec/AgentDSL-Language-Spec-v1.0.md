...
workflowBody    ::= descriptionProp
                   | agentsBlock
                   | flowBlock ;
agentsBlock     ::= "agents" "{" { "ref" STRING } "}" ;
flowBlock       ::= "flow" "{" { flowNode } "}" ;
flowNode        ::= ( "start" | "node" ) "(" STRING ")" "{" nodeBody "}" ;
nodeBody        ::= "onResult" CLOSURE
                   | "onComplete" CLOSURE ;
...
| `ref`      | `ref(String name): AgentRef`         | 引用已注册的 Agent       |
| `include`  | `include(String name): void`         | 引用已注册的 Tool        |
| `route`    | `route(String agentName): void`      | 工作流中路由到指定 Agent |
| `done`     | `done(): void`                       | 标记工作流节点完成       |
...
| Provider ID   | 目标 LangChain4j 类  | 必填配置            |
| ------------- | -------------------- | ------------------- |
| `"openai"`    | `OpenAiChatModel`    | `apiKey`            |
| `"ollama"`    | `OllamaChatModel`    | `baseUrl`           |
| `"dashscope"` | `QwenChatModel`      | `apiKey`            |
| `"zhipu"`     | `ZhipuAiChatModel`   | `apiKey`            |
| `"deepseek"`  | 通过 OpenAI 兼容接口 | `apiKey`, `baseUrl` |

> 系统应支持通过 SPI 机制注册自定义 Provider。
...
workflow("support-pipeline") {
    ...
    flow {
        start("classifier") {
            onResult { result ->
                when (result.level) {
                    "L1" -> route("support-agent")
                    "L2" -> route("escalation-agent")
                }
            }
        }
...
| `ADSL-002` | 编译期 | 未知关键字                              |
| `ADSL-003` | 编译期 | 类型不匹配（如 temperature 传入字符串） |
...
| 版本             | 内容                                                | 兼容性     |
| ---------------- | --------------------------------------------------- | ---------- |
| **v1.0**（当前） | `agent`, `tool`, 基础 `workflow`                    | 基线       |
| **v1.1**         | `outputSchema` 增强、`guardrails` 扩展              | 向后兼容   |
...
| `tools { include "name" }`             | `AiServices.builder().tools(toolObj).build()`         |
| `tool("name") { execute { ... } }`     | 实现 `ToolSpecification` + `ToolExecutor`             |
